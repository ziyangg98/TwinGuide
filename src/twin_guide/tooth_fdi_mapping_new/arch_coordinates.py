...
def _fit_arch(lr: np.ndarray, ap: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    segment = np.linalg.norm(
        np.diff(np.column_stack([curve_lr, curve_ap]), axis=0), axis=1
    )
...
def _pca_axis_permutation_candidates(
    ...
    guide_direction = (
        guide_delta / guide_norm if guide_norm > 1.0e-9 else None
    )
    ...
    for occlusal_index in (0, 1):
        ...
        signed_alignment = (
            float(e_occ @ guide_direction)
            if guide_direction is not None else 0.0
        )
...
def build_arch_frame_candidates(
    ...
    *,
    ...
    for axis_candidate in axis_candidates:
        ...
        names = (
            ("confirmed",)
            if explicit is not None else tuple(
                f"pca_occ_minor_{axis_index}_lr_{label}"
        ...
        for sign, name in zip(signs, names, strict=True):
            ...
            frames.append(ArchFrame(
                ...
                pca_occlusal_axis_index=(
                    int(axis_index) if axis_index is not None else None
                ),
