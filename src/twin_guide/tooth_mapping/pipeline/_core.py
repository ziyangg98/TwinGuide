...
def rounded(value: np.ndarray | float, digits: int = 6) -> float | list[object]:
    """内部算法说明。"""
...
class CurveModel:
    ...
    s: np.ndarray
    apex_index: int
    lr_to_ap: PchipInterpolator
    lr_to_s: PchipInterpolator
...
def _fit_curve(lr: np.ndarray, ap: np.ndarray) -> CurveModel:
    ...
    median_ap = []
    used_lr = []
    for left, right, center in zip(edges[:-1], edges[1:], centres, strict=False):
...
def guide_physical_coverage_top(
    ...
    return {
        ...
        "section_contour_count": len(section.discrete),
        "accepted_closed_exterior_contour_count": len(candidates),
        ...
        "exterior_component_point_count": len(exterior_component),
