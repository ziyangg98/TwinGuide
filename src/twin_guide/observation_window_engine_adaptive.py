...
def validate_contour_window_samples(window: dict[str, object]) -> None:
    ...
    internal_failures = [
        index for index in failed_indices if mapped_indices[0] < index < mapped_indices[-1]
    ]
    if internal_failures or mapped_indices != list(
        range(mapped_indices[0], mapped_indices[-1] + 1)
    ):
        raise RuntimeError(
            f"window {window['id']} has internal unmapped sections: {internal_failures}"
        )
    ...
    return (
        ...
        filled_overcut,
        {
            ...
            "nearest_following_wall_clearance_mm": (
                float(np.min(following_wall_clearances)) if following_wall_clearances else -1.0
            ),
...
def structured_prism(outer: np.ndarray, inner: np.ndarray) -> trimesh.Trimesh:
    ...
    for column in range(columns - 1):
        ...
        quad(
            outer_index(rows - 1, column),
            inner_index(rows - 1, column),
            inner_index(rows - 1, column + 1),
            outer_index(rows - 1, column + 1),
        )
    for row in range(rows - 1):
        quad(
            outer_index(row, 0),
        ...
        quad(
            outer_index(row, columns - 1),
            outer_index(row + 1, columns - 1),
            inner_index(row + 1, columns - 1),
            inner_index(row, columns - 1),
        )
    mesh = trimesh.Trimesh(
...
def build_grid_cutter(
    ...
    wall_thickness, applied_overcut, thickness_report = adaptive_wall_thickness(
        ...
        ray_entry_tolerance_mm,
        wall_overcut_mm,
    )
    outer = cutter_surface + outward_margin_mm * normals
    inner = cutter_surface - (wall_thickness[:, :, None] + applied_overcut[:, :, None]) * normals
...
def axis_sweep_axis_points(
    ...
    if additions.shape != (row_count,):
        raise RuntimeError("local_axis_drop_additions_mm must contain one value per axis row")
...
def build_axis_sweep_cutter(
    ...
    inward_bisector = unit(
        math.cos(middle_angle) * zero_direction + math.sin(middle_angle) * exterior_direction
    )
    ...
    return (
        ...
        outer,
        {
            ...
            "locally_corrected_axis_row_count": int(np.count_nonzero(local_drop_additions > 1e-9)),
        },
    )
...
def render_preview(
    ...
    for index, (title, first, second) in enumerate(
        ...
        axis.scatter(
            source_points[:, first], source_points[:, second], s=0.06, c="#94a3b8", alpha=0.05
        )
        axis.scatter(
            result_points[:, first], result_points[:, second], s=0.08, c="#c6a66b", alpha=0.24
        )
        axis.scatter(
            cutter_points[:, first], cutter_points[:, second], s=0.06, c="#dc2626", alpha=0.06
        )
        for name, surface in surfaces.items():
            axis.plot(
                surface[:, 0, first], surface[:, 0, second], linewidth=2.0, label=f"{name} top"
            )
...
def _run_once(args: argparse.Namespace) -> dict[str, object]:
    ...
    output_dir = (
        ...
        if args.output_dir
        else case_dir / "输出/mapped_observation_windows"
    )
    ...
    for item in args.window_wall_overcut_mm or []:
        ...
        if not separator or not window_id:
            raise RuntimeError("--window-wall-overcut-mm must use WINDOW_ID=MM syntax")
        try:
    ...
    for window in selected_windows:
        ...
        if opening_geometry == "axis_sweep":
            ...
            profiles = np.asarray(
                [item["window_profile_global_mm"] for item in window["samples"]], dtype=float
            )
            outward = np.asarray(
                [item["local_outward_global"] for item in window["samples"]], dtype=float
            )
            cutter, surface, wall_report = build_grid_cutter(
                ...
                outward,
                e_occ,
                bool(window["top_open"]),
                args.top_extension_mm,
                args.side_extension_mm,
                args.outward_margin_mm,
                wall_overcut_mm,
                args.maximum_wall_thickness_mm,
                args.ray_entry_tolerance_mm,
                args.union_batch_size,
            )
            ...
            raise RuntimeError(
                f"window {window['id']} has unsupported opening geometry {opening_geometry!r}"
            )
    ...
    has_local_axis_correction = any(
        np.any(
            np.asarray(
                window.get("axis_sweep", {}).get("local_axis_drop_additions_mm", []),
                dtype=float,
            )
            > 1e-9
        )
    ...
    if has_local_axis_correction:
        ...
        result_all = regularize_manifold(boolean("difference", [result_all, combined]))
    ...
    visibility_reports = {}
    axis_window_ids = {
        item["id"] for item in window_reports if item["opening_geometry"] == "axis_sweep"
    }
    if axis_window_ids:
    ...
    for mapped_window, window_report in zip(selected_windows, window_reports, strict=False):
        if window_report["opening_geometry"] == "axis_sweep":
            ...
            continue
        crest = np.asarray(
            [item["true_top_global_mm"] for item in mapped_window["samples"]], dtype=float
        )
        dense_crest = []
    ...
    qa = {
        ...
        "adaptive_wall_thickness_raycast_succeeded": all(
            bool(
                item["adaptive_wall_thickness"].get(
        ...
        "adaptive_overcut_preserves_following_wall_clearance": all(
            ...
            or item["adaptive_wall_thickness"]["minimum_clearance_after_overcut_mm"] >= 0.1 - 1e-6
            for item in window_reports
        ),
    ...
    if not qa_passed and not args.write_failed_qa_artifacts:
        raise RuntimeError(
            "mapped observation-window cut failed QA:\n"
            + json.dumps(
                {
                    "QA": qa,
                    ...
                    "adaptive_wall_thickness": {
                        item["id"]: item["adaptive_wall_thickness"] for item in window_reports
                    },
    ...
    scene = trimesh.Scene()
    scene.add_geometry(
        colored(source, (148, 163, 184, 35)), node_name="source_guide", geom_name="source_guide"
    )
    scene.add_geometry(
        colored(result, (198, 166, 107, 210)), node_name="cut_result", geom_name="cut_result"
    )
    scene.add_geometry(
        colored(combined, (220, 38, 38, 45)), node_name="window_cutter", geom_name="window_cutter"
    )
...
def _local_axis_failure_rows(
    ...
    reasons = []
    minimum_clearance = float(window_report.get("minimum_removed_axis_clearance_mm", math.inf))
    clearance_threshold = float(window_report.get("axis_clearance_threshold_mm", 0.15))
    if minimum_clearance < clearance_threshold:
        failed_rows.update(
            int(value) for value in window_report.get("axis_rows_below_clearance_threshold", [])
        )
        reasons.append("axis_clearance")

    visibility = window_report.get("tooth_visibility", {})
    if isinstance(visibility, dict):
        visible_fraction = float(visibility.get("axis_rows_with_visible_dental_fraction", 1.0))
        if visible_fraction < args.minimum_axis_visibility_row_fraction:
            failed_rows.update(
                int(value) for value in visibility.get("axis_rows_without_visible_dental", [])
            )
        ...
        if (
            len(corridor_by_row)
            and float(np.min(corridor_by_row)) < args.minimum_axis_clear_corridor_fraction
        ):
            failed_rows.update(
                int(value)
...
def _smoothed_local_drop_additions(
    ...
    for failed_row in failed_rows:
        ...
        for distance in range(1, transition_rows + 1):
            transition = increment_mm * (1.0 - distance / float(transition_rows + 1))
...
def _run_with_local_failure_correction(
    ...
    mapping_windows = {
        str(window["id"]): window for window in corrected_mapping["observation_windows"]
    }
    adaptations = {}
    for window_report in initial_report["windows"]:
        ...
        definition["local_axis_drop_additions_mm"] = [float(value) for value in additions]
        definition["maximum_effective_axis_drop_mm"] = float(definition["axis_drop_mm"]) + float(
            np.max(additions)
        )
        definition["local_failure_adaptation"] = {
            ...
            "corrected_axis_rows": [int(index) for index in np.flatnonzero(additions > 1e-9)],
        }
...
def _run_with_local_failure_target_sequence(
    ...
    targets = tuple(
        float(value) for value in getattr(args, "local_failure_drop_target_mm", None) or ()
    )
    if not targets:
        return _run_with_local_failure_correction(args)
    if float(args.local_failure_drop_increment_mm) != 0.0:
        raise RuntimeError("local failure drop targets and increment mode are mutually exclusive")
    ...
    mapping_windows = {
        str(window["id"]): window for window in working_mapping["observation_windows"]
    }
    ...
    for attempt_index, target_mm in enumerate(targets, 1):
        ...
        for window_report in current_report["windows"]:
            ...
            definition["local_axis_drop_additions_mm"] = [float(value) for value in additions]
            definition["maximum_effective_axis_drop_mm"] = baseline_drop_mm + float(
                np.max(additions)
            )
            adaptation = {
                "failed_axis_rows": failed_rows,
                ...
                "transition_rows": int(args.local_failure_transition_rows),
                "corrected_axis_rows": [int(index) for index in np.flatnonzero(proposed > 1e-9)],
            }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--minimum-axis-visibility-row-fraction", type=float, default=0.50)
    parser.add_argument("--minimum-axis-clear-corridor-fraction", type=float, default=0.95)
    ...
    parser.add_argument("--volume-identity-relative-tolerance", type=float, default=1e-4)
    parser.add_argument(
