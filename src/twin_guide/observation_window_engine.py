...
from twin_guide.tooth_mapping.pipeline import load_mesh, rounded, unit
...
def adaptive_wall_thickness(
    ...
    return filled, filled_overcut, {
        ...
        "following_wall_ray_count": int(len(following_wall_clearances)),
...
def build_axis_sweep_cutter(
    ...
    return cutter, outer, {
        ...
        "supported_axis_row_count": int(len(supported_rows)),
...
def axis_sweep_tooth_visibility(
    ...
    return {
        "visibility_ray_count": int(len(origins)),
        ...
        "axis_row_count": int(len(surface)),
...
def _run_once(args: argparse.Namespace) -> dict[str, object]:
    ...
    if axis_window_ids:
        ...
        for mapped_window, window_report in zip(selected_windows, window_reports):
    ...
    for mapped_window, window_report in zip(selected_windows, window_reports):
        if window_report["opening_geometry"] == "axis_sweep":
            ...
            for row, (start, end) in enumerate(zip(axis_points[:-1], axis_points[1:])):
        ...
        dense_crest = []
        for start, end in zip(crest[:-1], crest[1:]):
            dense_crest.extend(
