...
停止。双导柱旋转时，橙色小圆只标记两根导柱的当前位置；选中任意一个后拖动旁边的
小箭头，两根导柱就会绕共同中心同步旋转。右侧可直接输入角度，或用 `±1°`、`±5°`和
“归零”按钮。拖动时按 `Shift` 精调，按 `Ctrl` 整度对齐，`Esc` 取消。
...
单根连接梁检查点；操作窗、双侧连接避让和支撑结构只重建其依赖项。“更新快速预览”
将已切导板、连接梁和导柱组合显示，不执行最后体素融合、通道复切、手机差集或 QA，因此只用于
快速确认位置和大体形态。“确认导出并检验”才生成可交付 STL，并执行完整融合、避让、修复和检验。
