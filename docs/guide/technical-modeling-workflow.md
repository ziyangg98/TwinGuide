...
生成结果是标准重建导管、原导板、主连接梁、按压梁与显式特殊结构
...
圆梁、强化脚、差集切除体、体素融合与 STL 序列化。它不读取 FDI
决定锚点，不在布尔失败时更换算法或策略。
...
第 1 阶段是当前稳定识别与标准重建入口；第 2–7 阶段仍标记为实验。
完整回归包括：

- Ruff 和全部单元测试通过；
- Sphinx 使用 `-E -W --keep-going` 严格构建通过；
- Blender 后端、端到端生成与最终 STL 验证通过；
- 12 个正式病例完整回归，多种植位不跨装配体配对；
...
| 导管识别、方向、C 口与标准重建 | {doc}`../process/stage-1-sleeves` |
