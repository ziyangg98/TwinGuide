...
| 文件 | 含义 | 使用方式 |
| --- | --- | --- |
| `twin_guide.stl` | 融合导板、按种植位参数重建的导柱和梁架，复切导孔/观察窗并执行手机避让后的最终封闭网格 | 用于制造和 `validate` |
...
| --- | --- | --- | --- |
| `stage-01-sleeve-reconstruction` | 传统模板圆环估计的导柱位姿、局部标架和按种植位重建的导柱 | 导柱数、导柱索引、各位置最终长度和导孔直径 | 蓝色传统模板与灰色重建导柱 |
