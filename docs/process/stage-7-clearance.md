...
**实现状态：实验。** 本阶段从真实手机 STL 构造轴向位移与旋转的二维
运动包络，并将它作为完整模型的差集切除体。
...
手机 STL 的全部连通分量都参与运动包络，且每个分量都必须是封闭体。
...
轴向范围相对当前止挡姿态定义，正方向沿导管顶部指向下部；默认范围
`[0, 0]` 与旧病例一致。非零范围按 `axial_step_mm` 采样，并与旋转角形成
笛卡尔积姿态。
...
其他分量只有在体积不超过
`handpiece_avoidance.fragment_volume_tolerance_mm3` 时才被视为数值碎片。
任一更大独立分量都使本阶段失败。
...
该量与轴向半步位移相加，作为自动插值安全补偿；再与用户配置的额外净距
相加后使用外包络膨胀核执行最终切除。
