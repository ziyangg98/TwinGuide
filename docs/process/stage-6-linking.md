...
`independent_bezier` 兼容策略则按旧 TwinGuideMerge 公式，从每个导管的四个
真实外壁 Q 点分别连接到导板锚点。
...
独立 Bézier 模式记录每个导管四根梁（双导管共八根）及各自名称。
...
7. 主连接梁先以牙列外扩体做差集，裁掉进入牙体保护空间的部分。
   ...
   `input` 模式不采用该规则，只删除体积小于 1.5 个融合体素的数值碎片；
   该阈值删除单体素闭合伪影，但保留具有实际体积的独立输入实体。
...
10. `independent_bezier` 使用固定手柄倍率 3.0 和 24 段采样生成三次 Bézier；
    为保持旧实现语义，不执行当前牙列保护裁切，也不生成导板端渐粗和贴合脚。
    该模式不接受需要连续多种植位路径的锚点拓扑。
