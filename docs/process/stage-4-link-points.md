...
1. 将导套 C 口方向投影到轴线法平面并取反，得到成对向外方向，并检查其远离双导管中点。
2. 直接使用参数化导管的恒定主体外壁区间；上下高度分别由端缘净距和连接梁半径确定。
3. 在同一条外侧母线上生成真实外壁接触点 Q，并按壁厚/预埋规则沿外法向偏移为梁中心线点 P。
...
为保持拓扑兼容，模式名仍为 `tooth_section_trajectory`，但在
...
旧版 `stations` 仍可读取，并会在内存中等价展开为独立锚点。
